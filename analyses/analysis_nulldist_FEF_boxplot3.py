...
def get_plots(k, fwhm=7): 
    ...
    nulls_3d            = glob.glob(base_dir+"/fsl_feat_*NULL*/mean/inv_pe1.fwhm{}.nii.gz".format(fwhm)) #fwhm7.L2D.columns_ev_30000_borders.downscaled2x_NN.nii.gz")
    feat_3d             = base_dir+"/fsl_feat_1010.L_FEF_pca10/mean/inv_pe1.fwhm{}.nii.gz".format(fwhm) #L2D.columns_ev_30000_borders.downscaled2x_NN.nii.gz")
    ...
    for uc in unq_roi_col: 
        ...
        hG, = plot([1,1],'g-')
        hR, = plot([1,1],'r-')
        legend((hB, hG, hR),('NULL', 'NULL_mean','VASO'), loc='lower right')
        ...
        plt.savefig(plot_dir+"/{}.nullRois.box6.png".format(save_tit))
